...
- "followup": the user refers to the existing panel verdict — asking why a
  panelist voted a certain way, what the numbers mean, hypotheticals about
  Tesla ("what if it drops 20%?"), pressure/safety questions in the ongoing
  conversation ("just decide for me", "what if I borrow?"), or any
  continuation of the discussion.${hasVerdict ? "" : "\n  NOTE: no verdict exists yet in this session, so NEVER return \"followup\" — pick \"new_verdict\" or \"out_of_scope\"."}
...
like "it/that/the numbers", "why did X say...", "what if..." hypotheticals
about Tesla, or "so should I/my mom buy it?" — it is "followup".
Choose "new_verdict" ONLY for a clear request to run a fresh judgment.

EXAMPLES (assuming a verdict exists):
- "Why did Buffett say no?" -> followup
- "What if it drops another 20%?" -> followup
- "Explain the sandbox numbers again" -> followup
- "Ok so should my mom buy it or not?" -> followup
- "Ok just decide for me" -> followup
- "What if I borrow money to buy more?" -> followup
...
Respond ONLY with JSON: {"route": "new_verdict" | "followup" | "out_of_scope"}
`.trim();
}
